...
public class P2PMessageUtils {
    private static final Logger LOG = LoggerFactory.getLogger(P2PMessageUtils.class);
    ...
    public static final String HUNDSUN_P2P_MSG_INSTITUTION_ID = "P0013000"; // 机构标识
    ...
    public static String getDecryptContent(String encryptContent) {
        ...
        String xmlContent = P2PMessageUtils.decryptContentByAES(encrypt[3], aesKey);//使用AES密钥解密发送内容
    ...
    public static String signXmlContent(String xmlContent) {
        return RSAUtils.sign(getGBKByte(xmlContent), getKey(P2P_RSA_PRIVATE_KEY));
    }
    ...
    public static boolean verifySign(String xmlContent, String sign) {
        return RSAUtils.verify(xmlContent.getBytes(Charset.forName("GBK")), getKey(HUNDSUN_RSA_PUBLIC_KEY), sign);
    }
    ...
    public static String decryptAESKeyByRSA(String encryptKey) {
        return Hex.encodeHexString(RSAUtils.decryptByPrivateKey(base64Decode(encryptKey), getKey(P2P_RSA_PRIVATE_KEY)));
    }
    ...
    private static byte[] getGBKByte(String value) {
        return value.getBytes(Charset.forName("GBK"));
    }

    private static String getGBKString(byte[] value) {
    ...
    public static String encryptContentByAES(String text, String key) {
        return base64Encoder(AESUtils.encrypt(text.getBytes(Charset.forName("GBK")), hexToByte(key)));
    }
    ...
    public static String decryptContentByAES(String text, String key) {
        return getGBKString(AESUtils.decrypt(base64Decode(text), hexToByte(key)));
    }
